...
并且，***Chromium 与 Node.js 共享同一份 V8 库***，在构建时通过设置 node_shared=true 及关闭 Node.js 自带 V8，使得渲染进程和主进程都使用 Chromium 打包的 V8 实现。

而在运行时，Electron 遵循 Chromium 的多进程架构：***每个渲染进程和主进程都会各自启动一套 V8 运行循环（runloop），彼此相互隔离***。
...
- **独立运行时环境**：渲染进程在启动时，会注入 Node.js 全局对象（process、Buffer、require 等）并初始化 libuv、Node 核心模块。***每个渲染进程既是一个 Chromium 渲染沙箱，也是一个完整的 Node.js 运行时。***
